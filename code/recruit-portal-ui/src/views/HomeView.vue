  ...
  <div class="home">
    ...
    <section class="hero">
      ...
      <div class="hero__content">
        <p class="hero__tagline" v-motion-fade>
          <span class="tagline-item">团结</span>
          <span class="tagline-sep">、</span>
          <span class="tagline-item">务实</span>
          <span class="tagline-sep">、</span>
          <span class="tagline-item">厚德</span>
          <span class="tagline-sep">、</span>
          <span class="tagline-item">尚道</span>
        </p>

        <h1 class="hero__title" v-motion-fade>
          <span class="hero__title-line">赋能人类</span>
          <span class="hero__title-line">太空活动</span>
        </h1>

        <p class="hero__subtitle" v-motion-fade>
          <span class="subtitle-line">构建连接太空的基础设施，以电推进动力与全域态势感</span>
          <span class="subtitle-line">知技术，重新定义空间探索的边界。</span>
        </p>
...
/* Hero — 官网对齐 */
.hero { position: relative; min-height: 100vh; display: flex; align-items: center; justify-content: center; overflow: hidden; padding: 100px 24px 72px; }
...
.hero__nebula { position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); width: 600px; height: 600px; background: radial-gradient(ellipse, rgba(95,184,214,0.06) 0%, transparent 70%); pointer-events: none; }
...
.hero__content { position: relative; z-index: 1; text-align: center; max-width: 800px; padding: 0 16px; }

/* 企业精神 tagline（官网元素） */
.hero__tagline { color: #5FB8D6; font-size: 14px; letter-spacing: 6px; margin-bottom: 16px; }
.tagline-item { color: #5FB8D6; }
.tagline-sep { color: rgba(95,184,214,0.4); margin: 0 4px; }

/* 主标题 */
.hero__title { font-size: clamp(40px, 8vw, 80px); font-weight: 700; line-height: 1.15; letter-spacing: 4px; margin-bottom: 16px; }
.hero__title-line { display: block; color: #fff; }

/* 副标题 */
.hero__subtitle { font-size: 15px; color: #9CA3AF; line-height: 1.8; max-width: 580px; margin: 0 auto 40px; }
.subtitle-line { display: block; }

/* CTA */
.hero__cta { display: inline-flex; align-items: center; gap: 12px; padding: 14px 48px; border-radius: 8px; background: linear-gradient(135deg, #5FB8D6, #6BB3FF); color: #0A0E17; font-size: 16px; font-weight: 700; text-decoration: none; letter-spacing: 4px; transition: transform 0.3s, box-shadow 0.3s; }
...
/* 公告 */
.announce { padding: 0 0 16px; }
.announce__list { display: flex; flex-direction: column; gap: 6px; }
.announce__item { display: flex; align-items: center; gap: 12px; padding: 10px 18px; background: #152535; border-radius: 8px; border-left: 3px solid transparent; transition: border-color 0.3s; }
...
.announce__text { flex: 1; font-size: 14px; color: #cccccc; }
...
.section-title__cn { font-size: 24px; font-weight: 700; color: #fff; letter-spacing: 2px; }
...
/* 岗位 */
.jobs { padding: 36px 0; }
.jobs__grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(360px, 1fr)); gap: 16px; }
.job-card { background: #152535; border-radius: 12px; padding: 24px; border: 1px solid rgba(95,184,214,0.08); border-left: 3px solid transparent; transition: border-color 0.3s, border-left-color 0.3s, transform 0.3s, box-shadow 0.3s; }
...
.job-card__header { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 12px; }
.job-card__title { font-size: 17px; font-weight: 600; }
...
.job-card__meta { display: flex; gap: 16px; margin-bottom: 10px; flex-wrap: wrap; }
...
.job-card__desc { font-size: 13px; color: #9CA3AF; line-height: 1.6; }

/* 关于我们 */
.about { padding: 36px 0; border-top: 1px solid rgba(95,184,214,0.08); }
.about__grid { display: grid; grid-template-columns: 1fr 1fr; gap: 48px; align-items: start; }
.about__desc { font-size: 14px; color: #9CA3AF; line-height: 1.8; margin-bottom: 12px; }
.about__values { display: flex; gap: 10px; margin-top: 20px; }
.about__value { padding: 6px 16px; border-radius: 6px; border: 1px solid rgba(95,184,214,0.2); color: #5FB8D6; font-size: 13px; letter-spacing: 2px; }
.about__biz-title { font-size: 17px; font-weight: 600; color: #fff; margin-bottom: 16px; }
.about__biz-grid { display: grid; grid-template-columns: 1fr; gap: 12px; }
.about__biz-card { background: #152535; padding: 16px; border-radius: 10px; border: 1px solid transparent; transition: border-color 0.3s; }
.about__biz-card:hover { border-color: rgba(95,184,214,0.2); }
.about__biz-icon { font-size: 22px; margin-bottom: 6px; }
.about__biz-card h4 { font-size: 14px; color: #fff; margin-bottom: 4px; }
.about__biz-card p { font-size: 12px; color: #9CA3AF; line-height: 1.5; }

/* 滚动动画 */
[v-motion-fade] { opacity: 0; transform: translateY(16px); transition: opacity 0.6s ease, transform 0.6s ease; }
...
@media (max-width: 767px) {
  ...
  .hero__tagline { font-size: 12px; letter-spacing: 4px; margin-bottom: 12px; }
  .hero__title { font-size: 32px; letter-spacing: 3px; margin-bottom: 12px; }
  .hero__subtitle { font-size: 14px; margin-bottom: 32px; padding: 0; }
  .hero__cta {
    width: 100%;
  ...
  .job-card { border-left-color: rgba(95, 184, 214, 0.2); padding: 18px 16px; border-radius: 10px; }
  ...
  .jobs__bottom-cta { display: flex; justify-content: center; margin-top: 16px; }
  .jobs__bottom-cta a { display: inline-flex; align-items: center; gap: 8px; padding: 12px 40px; border-radius: 10px; border: 1px solid rgba(95, 184, 214, 0.4); color: #5FB8D6; font-size: 15px; font-weight: 600; letter-spacing: 2px; text-decoration: none; transition: background 0.2s, border-color 0.2s; }
  .jobs__bottom-cta a:active { background: rgba(95, 184, 214, 0.1); border-color: #5FB8D6; }

  .about { padding: 28px 0 40px; border-top: none; position: relative; }
  ...
  .about__desc { font-size: 13px; line-height: 1.7; margin-bottom: 8px; }
  .about__values { display: grid; grid-template-columns: 1fr 1fr; gap: 8px; }
  .about__value { display: flex; flex-direction: column; align-items: center; gap: 4px; padding: 10px 8px; background: rgba(21, 37, 53, 0.6); border: 1px solid rgba(95, 184, 214, 0.15); border-radius: 8px; font-size: 13px; letter-spacing: 2px; }
  .about__value::before { content: ''; display: block; width: 5px; height: 5px; border-radius: 50%; background: #5FB8D6; opacity: 0.6; }
  .about__biz-grid { display: flex; gap: 10px; overflow-x: auto; scroll-snap-type: x mandatory; -webkit-overflow-scrolling: touch; scrollbar-width: none; padding-bottom: 4px; }
  .about__biz-grid::-webkit-scrollbar { display: none; }
  .about__biz-card { flex: 0 0 75vw; max-width: 300px; scroll-snap-align: start; padding: 16px 14px; }
